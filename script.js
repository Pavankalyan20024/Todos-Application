...
const ROLE_PERMISSIONS = Object.freeze({
  admin: { createMember: true, editMember: true, updateMember: true, deleteMember: true, importExcel: true, clearData: true },
  manager: { createMember: false, editMember: true, updateMember: true, deleteMember: false, importExcel: false, clearData: false },
  employee: { createMember: false, editMember: false, updateMember: false, deleteMember: false, importExcel: false, clearData: false }
});

function hasPermission(permission) { return Boolean(ROLE_PERMISSIONS[currentUser?.accessLevel?.toLowerCase()]?.[permission]); }
function denyUnless(permission) { if (hasPermission(permission)) return true; showToast("You do not have permission to perform this action.", "error"); return false; }
function applyPermissionUI() {
  $("#addMemberBtn").hidden = !hasPermission("createMember");
  $$('[data-import]').forEach(control => control.hidden = !hasPermission("importExcel"));
  $("#clearDataBtn").hidden = !hasPermission("clearData");
  const dataCard = $(".settings-data-card");
  if (dataCard) dataCard.hidden = !hasPermission("importExcel") && !hasPermission("clearData");
}
...
function getAuthSession() { return currentUser; }
async function api(path, options = {}) {
  ...
  if (!response.ok || (response.status !== 204 && !contentType.includes("application/json"))) { const message = response.status === 403 ? "You do not have permission to perform this action." : data?.message || (response.status >= 500 ? "Unable to complete the request. Please try again." : "Unable to connect to the TaskFlow service. Please run TaskFlow through its application server."); if (response.status === 403) showToast(message, "error"); if (response.status === 401 && currentUser) showLoginPage(); const error = new Error(message); error.status = response.status; throw error; }
...
function showLoginPage() { currentUser = null; $("#taskflowApp").hidden = true; $("#loginPage").hidden = false; showAuthView("login"); $("#profileMenu").classList.remove("open"); $("#loginPassword").value = ""; $("#loginPassword").type = "password"; $("#toggleLoginPassword").setAttribute("aria-label", "Show password"); document.title = "TaskFlow — Login"; setTimeout(() => $("#loginEmail").focus(), 0); }
function useAuthenticatedUser(user) { currentUser = { id: user.id, name: user.name, email: user.email, role: user.role || "Employee", accessLevel: String(user.accessLevel || "employee").toLowerCase(), authProvider: user.authProvider }; settings.name = user.name; settings.email = user.email; settings.role = currentUser.role; saveSettings(); applySettings(); applyPermissionUI(); }
async function loadMembers() { projectData = (await api("/api/members")).map(normalizeMemberRecord); refreshAll(); }
async function showApplication(user = currentUser) { if (user) useAuthenticatedUser(user); $("#loginPage").hidden = true; $("#taskflowApp").hidden = false; switchPage("dashboard", "replace"); try { await loadMembers(); } catch (error) { console.error(error); showToast("Unable to load team members.", "error"); } }
...
function normalizeAccessLevel(value) { const level = String(value || "employee").trim().toLowerCase(); return ["admin", "manager", "employee"].includes(level) ? level : "employee"; }
function accessLevelLabel(value) { const level = normalizeAccessLevel(value); return level[0].toUpperCase() + level.slice(1); }
function normalizeMemberRecord(record = {}) { return { ...record, role: String(record.role || "").trim(), accessLevel: normalizeAccessLevel(record.accessLevel), skills: normalizeSkills(record.skills), technologiesUsed: normalizeTechnologies(record.technologiesUsed), blockers: normalizeBlockers(record.blockers) }; }
...
const COLUMN_ALIASES = {
  ...
  accessLevel: ["access level", "access role", "system role", "permission level"],
...
function normalizeExcelRow(row, columnMap) {
  const values = { id: "", teamMember: "", project: "", role: "", accessLevel: "employee", skills: [], technologiesUsed: [], currentWork: "", workingWith: "", contribution: "", status: "", expectedCompletionDate: "", blockers: [] };
  ...
  values.accessLevel = normalizeAccessLevel(values.accessLevel);
...
async function importExcelFile(file) {
  if (!file) return;
  if (!hasPermission("importExcel")) { $("#excelInput").value = ""; showToast("You do not have permission to import Excel data.", "error"); return; }
  ...
  } catch (error) { console.error("Excel import failed:", error.message); showToast(error.status === 403 ? "You do not have permission to import Excel data." : error.message || "Excel import failed. Please check the file and try again.", "error", 7000); }
...
function renderCurrentTeamInvolvement() {
  ...
  root.innerHTML = `<div class="involvement-table"><div class="involvement-head"><span>Team Member</span><span>Project / Workstream</span><span>Role</span><span>Access Level</span><span>Technologies Used</span><span>Skills / Expertise</span><span>Status</span><span>Blockers</span><span>Working With</span><span>Current Work</span><span></span></div>${latest.map(item => `<article class="involvement-row"><div class="member-cell" data-label="Team Member"><span class="member-avatar">${initials(item.teamMember)}</span><span><strong>${escapeHTML(item.teamMember)}</strong><small>${escapeHTML(item.id)}</small></span></div><strong data-label="Project / Workstream">${escapeHTML(item.project)}</strong><span data-label="Role">${escapeHTML(item.role || "—")}</span><span data-label="Access Level"><span class="access-level-badge access-${normalizeAccessLevel(item.accessLevel)}">${accessLevelLabel(item.accessLevel)}</span></span><div class="technologies-column" data-label="Technologies Used">${technologyTagsMarkup(item.technologiesUsed)}</div><div class="skills-column" data-label="Skills / Expertise">${skillsMarkup(item.skills)}</div><span class="status ${statusClass(item.status)}" data-label="Status">${escapeHTML(item.status)}</span><span class="blocker-text ${hasBlocker(item.blockers) ? "has" : ""}" data-label="Blockers">${escapeHTML(blockerText(item.blockers))}</span><div class="working-with-column" data-label="Working With">${escapeHTML(item.workingWith || "—")}</div><div class="current-work-column" data-label="Current Work">${escapeHTML(item.currentWork || "—")}</div><div class="involvement-actions-column" data-label="Actions"><button class="row-menu" data-dashboard-details="${escapeHTML(item.id)}">⋮</button></div></article>`).join("")}</div>`;
}
...
$("#role").closest("label").insertAdjacentHTML("afterend", `<label>Access Level<select id="memberAccessLevel" required><option value="employee">Employee</option><option value="manager">Manager</option><option value="admin">Admin</option></select><small class="field-error" data-error-for="memberAccessLevel"></small></label>`);
...
function memberCard(item) { const edit = hasPermission("editMember") ? `<button data-member-action="edit">Edit</button>` : ""; const remove = hasPermission("deleteMember") ? `<button data-member-action="delete" class="delete">Delete</button>` : ""; return `<article class="member-card" data-id="${escapeHTML(item.id)}"><div class="member-card-head"><span class="member-avatar large">${initials(item.teamMember)}</span><div><h3>${escapeHTML(item.teamMember)}</h3><small>${escapeHTML(item.id)}</small></div><div class="member-menu"><button data-member-action="menu" aria-label="Open actions">⋮</button><div class="task-action-menu"><button data-member-action="view">View Details</button>${edit}${remove}</div></div></div><div class="member-project"><strong>${escapeHTML(item.project)}</strong><span>${escapeHTML(item.role)}</span></div><p class="current-work">${escapeHTML(item.currentWork)}</p><div class="member-meta"><span class="status ${statusClass(item.status)}">${escapeHTML(item.status)}</span><span>▣ ${formatDate(item.expectedCompletionDate)}</span></div><div class="card-blocker"><strong>Blockers:</strong> <span class="${hasBlocker(item.blockers) ? "has" : ""}">${escapeHTML(blockerText(item.blockers))}</span></div></article>`; }
...
function openMemberModal(id = null) {
  if (!denyUnless(id ? "editMember" : "createMember")) return;
  $("#memberForm").reset(); clearMemberErrors(); resetBlockerPicker(); resetMemberSkills(); $("#originalMemberId").value = ""; $("#memberAccessLevel").value = "employee"; $("#memberAccessLevel").disabled = false; $("#memberModalTitle").textContent = id ? "Edit Member" : "Add Member"; $("#memberSubmitBtn").textContent = id ? "Update Member" : "Save Member";
  if (id) { const item = projectData.find(row => row.id === id); if (!item) return; $("#originalMemberId").value = item.id; $("#memberId").value = item.id; $("#teamMember").value = item.teamMember; $("#project").value = item.project; $("#role").value = item.role; $("#memberAccessLevel").value = normalizeAccessLevel(item.accessLevel); $("#memberAccessLevel").disabled = currentUser?.accessLevel !== "admin"; resetMemberSkills(item.skills); $("#technologiesUsed").value = normalizeTechnologies(item.technologiesUsed).join(", "); $("#currentWork").value = item.currentWork; $("#workingWith").value = item.workingWith || ""; $("#contribution").value = item.contribution || ""; $("#memberStatus").value = normalizeStatus(item.status); $("#completionDate").value = item.expectedCompletionDate; populateBlockerPicker(item.blockers); }
  $("#memberModal").classList.add("open"); document.body.classList.add("member-modal-open"); setTimeout(() => $("#memberId").focus(), 80);
}
async function saveMember(event) {
  event.preventDefault(); clearMemberErrors();
  const original = $("#originalMemberId").value;
  if (!denyUnless(original ? "updateMember" : "createMember")) return;
  const originalRecord = original ? projectData.find(row => row.id === original) : null;
  const selectedAccessLevel = $("#memberAccessLevel").value;
  const item = { id: $("#memberId").value.trim(), teamMember: $("#teamMember").value.trim(), project: $("#project").value.trim(), role: $("#role").value.trim(), accessLevel: selectedAccessLevel, skills: [...memberFormSkills], technologiesUsed: normalizeTechnologies($("#technologiesUsed").value), currentWork: $("#currentWork").value.trim(), workingWith: $("#workingWith").value.trim(), contribution: $("#contribution").value.trim(), status: normalizeStatus($("#memberStatus").value), expectedCompletionDate: $("#completionDate").value, blockers: selectedBlockers(), createdAt: originalRecord?.createdAt || Date.now() };
  const checks = [["memberId", item.id, "Employee ID is required."], ["teamMember", item.teamMember, "Team member name is required."], ["project", item.project, "Project or workstream is required."], ["role", item.role, "Role is required."], ["memberAccessLevel", ["employee", "manager", "admin"].includes(selectedAccessLevel), "Please select a valid access level."], ["technologiesUsed", item.technologiesUsed.length, "Technologies Used is required."], ["currentWork", item.currentWork, "Current Work is required."], ["memberStatus", item.status, "Status is required."], ["completionDate", item.expectedCompletionDate, "Expected Completion Date is required."]];
...
function applySettings() {
  ...
  roleSelect.value = settings.role; $("#accessLevelInput").value = accessLevelLabel(currentUser?.accessLevel); $("#themeToggle").checked = settings.theme === "dark"; $$('[data-notification-key]').forEach(toggle => toggle.checked = Boolean(settings[toggle.dataset.notificationKey])); $("#languageSelect").value = settings.language; document.body.classList.toggle("dark", settings.theme === "dark");
}
function setTheme(theme) { settings.theme = theme; saveSettings(); applySettings(); renderStatusChart(); renderReports(); }
function clearAllProjectData() { if (!denyUnless("clearData")) return; askConfirmation("Clear All Data?", "This will permanently remove all imported team project data. Your profile and preferences will remain.", async () => { try { await api("/api/members", { method: "DELETE" }); await loadMembers(); showToast("All team project data cleared."); } catch (error) { showToast(error.message || "Unable to clear team project data.", "error"); } }, "Clear Data"); }
...
$$('[data-import]').forEach(button => button.addEventListener("click", () => { if (hasPermission("importExcel")) $("#excelInput").click(); else showToast("You do not have permission to import Excel data.", "error"); }));
...
$("#profileForm").addEventListener("submit", async event => { event.preventDefault(); const name = $("#nameInput").value.trim(), email = $("#emailInput").value.trim(), jobTitle = $("#roleSelect").value, emailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email); $("#nameError").textContent = name ? "" : "Full Name is required."; $("#emailError").textContent = email ? (emailValid ? "" : "Enter a valid email address.") : "Email Address is required."; if (!name || !emailValid) return; const button = event.target.querySelector('button[type="submit"]'); button.disabled = true; button.textContent = "Saving..."; try { const user = await api("/api/profile", { method: "PUT", body: JSON.stringify({ name, email, role: jobTitle }) }); settings.role = jobTitle; useAuthenticatedUser(user); showToast("Profile updated successfully."); } catch (error) { showToast(error.message || "Unable to update profile.", "error"); } finally { button.disabled = false; button.textContent = "Save Changes"; } });
